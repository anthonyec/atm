...
const parseDatasetItem = function(datasetItem) {
  if (!datasetItem || !datasetItem['ns3:Value'] || !datasetItem['ns3:Value'][0]) {
    return null;
  }
...
exports.parseDataResponse = function(xml, variableIds, fields = []) {
  ...
  if (!datasetItems['ns3:DatasetItem']) {
    throw new ReferenceError('XML is missing "ns3:DatasetItems" element');
  }

  //  at this point we can have multiple DatasetItem
  const variables = variableIds.split(',');
  const data = datasetItems['ns3:DatasetItem'].reduce((prev, datasetItem, index) => {
    prev[variables[index]] = parseDatasetItem(datasetItem);
    return prev;
  },{});
  return data;
